...
def paginate_book(filename):
    ...
    try:
        ...
        book_pages = [["Lỗi: Không thể đọc tệp văn bản."]]
...
def ui_refresh():
    with Image.new("RGB", (WIDTH, HEIGHT)) as img:
        ...
        if current_state == "MENU":
            ...
            if book_pages:
                lines = book_pages[current_page_idx]
...
def touch_callback(x, y):
    ...
    if current_state == "MENU":
        ...
        if y > 190:
            if x < 120: current_page_idx = max(0, current_page_idx - 1)
            elif x > 200: current_page_idx = min(len(book_pages)-1, current_page_idx + 1)
            ui_refresh()

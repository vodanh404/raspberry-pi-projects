...
#
# Cập nhật bởi Gemini: Thêm tham số aBgColor để hỗ trợ màu nền cho văn bản
...
class TFT(object):
    ...
    # @micropython.native
    def text(self, aPos, aString, aColor, aFont, aSize=1, nowrap=False, aBgColor=None):
        ...
        px, py = aPos
        width = wh[0] * aFont["Width"]
        for c in aString:
            self.char((px, py), c, aColor, aFont, wh, aBgColor)
            px += width + 1
    ...
    # @micropython.native
    def char(self, aPos, aChar, aColor, aFont, aSizes, aBgColor=None):
        ...
        if (startchar <= ci <= endchar):
            ...
            # Nếu có màu nền, vẽ hình chữ nhật nền trước
            if aBgColor is not None:
                self.fillrect(aPos, (fontw * aSizes[0], fonth * aSizes[1]), aBgColor)
